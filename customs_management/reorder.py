...
def _reorder_item():
    ...
    def add_to_material_request(**kwargs):
        # print("\nadd_to_material_request",kwargs.get('item_code'))
        ...
        # print("item_warehouse_projected_qty",item_warehouse_projected_qty.get(kwargs.item_code, {}).get(kwargs.warehouse_group),kwargs.warehouse_group)
        ...
        if (reorder_level or reorder_qty) and projected_qty <= reorder_level:
            # print(f"Projected qty: {projected_qty} <=  reorder_level: {reorder_level} for ====> {kwargs.item_code}")
            ...
            material_requests[kwargs.material_request_type].setdefault(company, []).append(
              ...
              pass
            #   print(f"Projected qty: {projected_qty} > reorder_level: {reorder_level} for xxx=> {kwargs.item_code}")

    items_to_consider = get_items_for_reorder()
    print("total items to consider", len(items_to_consider.items()))
    ...
    total_count=0
    count=0
    for key,item in items_to_consider.items():
        ...
        # total_count+=count
        count=0
        print("new dict",new_dict.keys(),len(new_dict.keys()))
        item_warehouse_projected_qty = get_item_warehouse_projected_qty(new_dict)



        for item_code, reorder_levels in new_dict.items():
        ...
        if material_requests:
            # print("material_requests first created",material_requests)
            mr= create_material_request(material_requests)
            print("\n\n\nmaterial req",mr)
            material_requests = {"Purchase": {}, "Transfer": {}, "Material Issue": {}, "Manufacture": {}}
            frappe.db.commit()
            # if total_count >= 1050:
...
def create_material_request(material_requests):
    ...
    # print("material requests create",material_requests)
    print("\nkeys:",material_requests.keys())
    ...
    for request_type in material_requests:
        ...
        for company in material_requests[request_type]:
            ...
            try:
                ...
                print("no of items with respect to warehouse",len(items))
                mr = frappe.new_doc("Material Request")
                mr.update(
                    {
                        "company": company,
                        "transaction_date": nowdate(),
                        "custom_auto_reorder": True,
